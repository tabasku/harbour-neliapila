...
class Storage:
    ...
    def insert_boards(self,boards_data,table_exists):
        ...
        for board in boards_data['boards']:
            if table_exists:
                cmd += '''
                INSERT OR REPLACE INTO BOARDS (BOARD,TITLE,META,PAGES,FAVORITE,DEFAULT_BOARD,LAST_USED)
                VALUES (
                "{board}",
                "{title}",
                "{meta}",
                ...
                '''.format(board=board['board'], title=board['title'],
                           meta=board['meta_description'],pages=board['pages'])
            else:
                cmd += '''
                INSERT OR REPLACE INTO BOARDS (BOARD,TITLE,META,PAGES)
                VALUES (
                "{board}",
                "{title}",
                "{meta}",
    ...
    def insert_board(self,board,title,table_exists):

        #favorite = self.db_query('SELECT FAVORITE FROM BOARDS WHERE BOARD = "{board}"'.format(board=board),"one")[0]
        #DEFAULT_BOARD = self.db_query('SELECT DEFAULT_BOARD FROM BOARDS WHERE BOARD = "{board}"'.format(board=board),"one")[0]

        if table_exists:
            cmd = '''
            INSERT OR REPLACE INTO BOARDS (BOARD,TITLE,FAVORITE,DEFAULT_BOARD)
            VALUES (
            "{board}",
            "{title}",
            (SELECT FAVORITE FROM BOARDS WHERE BOARD = "{board}"),
            ...
            '''.format(board=board, title=title)
            #print(self.db_query('SELECT FAVORITE FROM BOARDS WHERE BOARD = "{board}"'.format(board=board),"one")[0])
        else:
            cmd = '''
            INSERT OR REPLACE INTO BOARDS (BOARD,TITLE)
            VALUES (
            "{board}",
            "{title}"
            );
