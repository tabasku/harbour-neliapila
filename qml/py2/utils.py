...
def parse_posts(list,board,post_replies=None):
 ...
 #   if post_replies:
  ...
  #      for key, value in post_replies:
    ...
    for post in list:
        ...
        if 'tim' in post:
            post['thumbUrl'] = "https://t.4cdn.org/"+ board +"/" + str(post['tim']) + "s.jpg"
            post['imgUrl'] = "https://i.4cdn.org/"+ board +"/" + str(post['tim']) + post['ext']
        elif 'filedeleted' in post:
        ...
        #Time stuff
        #FMT = '%Y-%m-%d %H:%M:%S'
        #post['post_time'] = datetime.datetime.fromtimestamp(post['time']).strftime(FMT)
        #post['current_time'] = datetime.datetime.now().strftime(FMT)
        #post['time_diff'] = datetime.datetime.strptime(post['current_time'], FMT) - datetime.datetime.strptime(post['post_time'], FMT)
