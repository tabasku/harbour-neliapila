    ...
    def parse_com(com):
        ...
        for untagged_url in fixlist:
            ...
            if "boards.4chan" not in untagged_url:
                if '?' in untagged_url:
                    untagged_url = untagged_url.replace('?', '\?')
                if '+' in untagged_url:
                    untagged_url = untagged_url.replace('+', '\+')
                com = re.sub(untagged_url + r'($|\s|[^a-zA-Z0-9_/?\u3000-\u303f\u3040-\u309f\u30a0-\u30ff\uff00-\uff9f\u4e00-\u9faf\u3400-\u4dbf])', fixed_url + r'\1', com)
